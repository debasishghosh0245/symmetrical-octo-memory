import java.util.*;
public class DetpthFirstTraversal {
	...
	public static void printPath(List<Edge>[] graph,int vertices,int source){
		Stack<Pair> stack=new Stack<>();
		stack.add(new Pair(source,source+""));
		...
		while(!stack.isEmpty()){
			...
			for(Edge edge:edges){
				if(visited[edge.neighbour]==false){
					stack.add(new Pair(edge.neighbour,pair.pathsofar+edge.neighbour));
				}
			}
		}	
	}
